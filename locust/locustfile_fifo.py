"""
Locust Load Test for FIFO Queue - Fixed Version
"""
...
task_types = []
all_submitted_tasks = []
completed_task_ids = set()  # Track which tasks are completed
api_host = None

class TaskQueueUser(HttpUser):
    """Simulates a client submitting tasks to the FIFO queue"""
            ...
            if response.status_code == 200:
                try:
                    ...
                    if status in ["success", "failed"]:
                        ...
                        completed_task_ids.add(task_info["id"])  # Track completion
                    else:
                        self.pending_tasks.append(task_info)
...
def poll_remaining_tasks(max_wait_time=600):
    ...
    # Find uncompleted tasks (not in completed set)
    uncompleted_tasks = [t for t in all_submitted_tasks 
...
def on_test_stop(environment, **kwargs):
    ...
    print("\n" + "="*60)
    print("FIFO QUEUE - FINAL LATENCY ANALYSIS")
    print("="*60)
